...
pub fn serialize_my_object(args: MyObject) -> Result<Vec<u8>, EncodeError> {
    ...
    write_my_object(args, &mut encoder)?;
...
pub fn deserialize_my_object(args: &[u8]) -> Result<MyObject, DecodeError> {
    let mut context = Context::new();
    context.description = "Deserializing object-type: MyObject".to_string();
    let mut reader = ReadDecoder::new(args, context);
    read_my_object(&mut reader)
}

pub fn read_my_object<R: Read>(reader: &mut R) -> Result<MyObject, DecodeError> {
